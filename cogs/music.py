...
class Music(commands.Cog):
	...
	async def _leave(self, ctx: commands.Context):
			...
			embed = discord.Embed(
					title="Disconnected from Music ", color=0xFF0000
			)
			await ctx.send(embed=embed)
	...
	async def _pause(self, ctx: commands.Context):
			...
			if ctx.voice_state.is_playing and ctx.voice_state.voice.is_playing():
				...
				await message.edit(content="Song Paused!")
	...
	async def _resume(self, ctx: commands.Context):
			...
			if ctx.voice_state.is_playing and ctx.voice_state.voice.is_paused():
				...
				await message.edit(content="Song Playing!")
				await ctx.message.add_reaction("⏯")
	...
	async def _stop(self, ctx: commands.Context):
			...
			if ctx.voice_state.is_playing:
					...
					await ctx.message.add_reaction("⏹")
					await ctx.send('Music Stopped!! :raised_hand:')
	...
	async def _skip(self, ctx: commands.Context):
			...
			if voter == ctx.voice_state.current.requester:
					...
					if total_votes >= round((len(members)-1)/2):
							await ctx.message.add_reaction("⏭")
							ctx.voice_state.skip()
							await ctx.send('Next song coming up')
					else:
							await ctx.send(
									f'Added Skip Vote! votes currently at {total_votes}/{round((len(members)-1)/2)}')

			else:
					await ctx.send("You have already voted to skip this song duh")
	...
	async def _remove(self, ctx: commands.Context, index: int):
			...
			await ctx.message.add_reaction("✅")
			await ctx.send('Song Succesfully removed :white_check_mark:')
