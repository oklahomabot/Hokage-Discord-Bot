...
approved_users = [569105874912804874]
...
class images(commands.Cog):
    ...
    '''
    @commands.command(aliases=['slide', 'slideshow'], hidden=True)
    async def slide_show(self, ctx, delay=1000):

    Creates a 'slide show' of fun_list images
    This uses the last image created from each
    of the fun_list commands

        im_list = []
        max_x, max_y = 0, 0
        background = Image.new('RGBA', (1200, 1200), 0)
        im_list.append(background)
        # Iterate over output folder and make image object list
        input_filenames = os.listdir(output_folder)
        for file in input_filenames:
            temp_im = Image.open(os.path.join(output_folder, file))
            temp_im = temp_im.copy()
            im_list.append(temp_im)

        outfile = os.path.join(output_folder, "slideshow.gif")
        im_list[0].save(outfile,
                        save_all=True, append_images=im_list[1:], optimize=False, duration=delay, loop=0, interlace=False, disposal=1)
        file = (discord.File(outfile))
        await ctx.send(f'Still a work in progress ... max_x {max_x} max_y {max_y}')
        # await ctx.send(file=file)
    '''
    ...
    async def delete_command(self, ctx, command_name='UNDEFINED-COMMAND'):
        '''
        DELETES all information for one fun image command from the database and image_dic
        '''
        if ctx.message.author.id not in approved_users:
            ctx.send(
                f'You are not authorized to run this command ... see approved_users list')
            return
    ...
    @commands.command(aliases=['testcircle', 'dryer'], hidden=True)
    async def test_circle(self, ctx, *, user=None):
...
def make_circle(im):
    ...
    im.putalpha(mask)
    # im.show()
    # print('circle created ...  take a look')
    return im
